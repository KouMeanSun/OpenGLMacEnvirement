...
-(void)loadMetal:(nonnull MTKView *)mtkView{
    ...
    //4.同步创建并返回渲染管线对象
    NSError *error = NULL;
    _piplineState = [_device newRenderPipelineStateWithDescriptor:pipelineStateDescriptor error:&error];
    //判断是否创建成功
    if(!_piplineState){
    ...
    _numberVertices = vertexData.length / sizeof(CCVertex);
...
-(void)mtkView:(MTKView *)view drawableSizeWillChange:(CGSize)size{
    //保存可绘制的大小，因为当我们绘制时，我们将把浙西信息传给顶点着色器
...
- (void)drawInMTKView:(MTKView *)view{
    ...
    //2.MTLRenderPassDescriptor:一组渲染目标，用作渲染通道生成的像素输出目标.
    //currentRenderPassDescriptor 从 从currentDrawable's texture,view's depth, stencil, and sample buffers and clear values.
