...
@implementation TriangleRender
{
    ...
    //我们的渲染管道有顶点着色器和片元着色器 他们存储在.metal shader 文件中
...
- (void)mtkView:(MTKView *)view drawableSizeWillChange:(CGSize)size{
    //保存可绘制的大小，因为当我们绘制时，我们将把这些值传给顶点着色器
...
- (void)drawInMTKView:(MTKView *)view{
    ...
    //3.MTLRenderPassDescriptor；一组渲染目标，用作渲染通道生成的像素的输出日志
    MTLRenderPassDescriptor *renderPassDescriptor = view.currentRenderPassDescriptor;
    //判断渲染目标是否为空
    if(renderPassDescriptor != nil){
        //4.创建渲染命令编辑器，这样我们才可以渲染到something
        ...
        [renderEncoder setVertexBytes:triangleVertices length:sizeof(triangleVertices) atIndex:TriangleVertexInputIndexVertices];
        //viewPortSize 数据
        //1) 发送到顶点着色函数中,视图大小
        //2) 视图大小内存空间大小
        //3) 对应的索引
        [renderEncoder setVertexBytes:&_viewportSize length:sizeof(_viewportSize) atIndex:TriangleVertexInputViewPortSize];
        //8.画出三角形的3个顶点
