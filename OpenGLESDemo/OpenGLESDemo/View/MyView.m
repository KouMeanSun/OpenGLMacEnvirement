...
-(void)renderLayer{
    ...
    //1)注意，第二个参数字符串必须和shaderv。vsh中的输入变量：position保持一致
    ...
    GLuint texCoordinate = glGetAttribLocation(self.myPrograme, "texCoordinate");
    //2)设置合适的格式从buffer里读取数据
    ...
    //参数5：stride,连续顶点属性之间的偏移量，默认为0；
    //参数6：指定一个指针，指向数组中的第一个顶点属性的第一个组件。默认为0
    glVertexAttribPointer(texCoordinate, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat)*5, (float *)NULL +3);
